 ...
 *  - Usa PUBLIC_URL_DEFAULT si no existe process.env.PUBLIC_URL
 *  - Endpoint /v1/config para que el cliente descubra la URL base
 ******************************************************************/
...
/* ---- 1.  URL por defecto (cámbiala y haz git push) ------------- */
...
/* ---- 4.  Endpoint “config” (la app lo llama al arrancar) ------- */
app.get('/v1/config', (req, res) => {
  ...
  res.json({ version: 1, apiBaseUrl: url });
});
...
/* ---- 8.  Arranque -------------------------------------------- */
app.listen(PORT, () =>
  console.log(`✅ Mock MOWIZ en http://localhost:${PORT}`),
);
